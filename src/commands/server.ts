...
export function registerServerCommand(program: Command): void {
  ...
  srv.command("start")
    .description("Start the AIHub server (background by default)")
    ...
    .action(async (opts: { port: string; foreground?: boolean }) => {
      ...
      if (!opts.foreground) {
        ...
        const child = spawn("node", [path.join(aihubDir, "dist", "bin", "aihub.js"), "server", "start", "-p", opts.port, "-f"], {
          cwd: aihubDir,
          detached: true,
        ...
        if (await api.health()) {
          log.success(`Server started (port ${opts.port}, log: ${logFile})`);
        } else {
